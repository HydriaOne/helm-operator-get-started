...
redeyes is a tiny web application made with Go 
...
```console
$ helm repo add sp https://stefanprodan.github.io/redeyes
$ helm upgrade my-release --install sp/redeyes 
```

The command deploys redeyes on the Kubernetes cluster in the default namespace.
...
The following tables lists the configurable parameters of the redeyes chart and their default values.
...
`image.repository` | image repository | `stefanprodan/redeyes`
...
`service.grpcService` | gPRC service name | `redeyes`
...
```console
$ helm install stable/redeyes --name my-release \
...
```console
$ helm install stable/redeyes --name my-release -f values.yaml
```
